...
!
  ...
  if ( ldfjac < m &
    .or. ftol < 0.0D+00 .or. xtol < 0.0D+00 .or. gtol < 0.0D+00 &
     .or. maxfev <= 0 .or. factor <= 0.0D+00 ) go to 300
...
!
  ...
  iflag = 0
!
!corrado-serve per applicare i parametri nel caso il chi non sia sceso
  iflag = 1
  call fcn ( m, n, x, fvec, fjac, ldfjac, iflag )
!
  if ( nprint > 0 ) then
...
subroutine lmder1 ( fcn, m, n, x, fvec, fjac, ldfjac, tol, diag, mode, info )
!
!*******************************************************************************
!
...
!    Output, real ( kind = 8 ) FVEC(M), the functions evaluated at the output X.
!
!    Output, real ( kind = 8 ) FJAC(LDFJAC,N), an M by N array.  The upper N by N 
!    submatrix contains an upper triangular matrix R with
...
!    or that the relative error between X and the solution is at
!    most TOL.
!
!    Output, integer INFO, error flag.  If the user has terminated execution, 
!    INFO is set to the (negative) value of IFLAG. See the description of FCN. 
...
!
!    Input/output, real ( kind = 8 ) R(LDR,N),the N by N matrix.  The full upper triangle
!    must contain the full upper triangle of the matrix R.  On output the 
!    full upper triangle is unaltered, and the strict lower triangle contains 
...
!
!    Output, real ( kind = 8 ) ACNORM(N), the norms of the corresponding columns of the 
